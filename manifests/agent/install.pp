...
class teamcity::agent::install {
  ...
  package { 'teamcityagent':
    install_options => ['-params','"',"serverurl=${teamcity::agent::server_url}","ownPort=${teamcity::agent::agent_port}","agentName=${teamcity::agent::agent_name}","agentDir=${teamcity::agent::install_dir}",'"',"--allow-empty-checksums"],
    provider        => 'chocolatey',
  }
